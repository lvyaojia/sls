...
def sample_machine_group(client, project) :
    ...
    machine_group = MachineGroupDetail("test-group", "ip", ["127.0.0.1", "127.0.0.2"], "Armory", {"externalName" : "test-1", "groupTopic":"yyy"})
    ...
    res = client.get_machine_group(project, "test-group")
    res.log_print()


    machine_group = MachineGroupDetail("test-group", "userdefined", ["127.0.0.1", "127.0.0.3"], "Armory", {"externalName" : "test-1", "groupTopic":"yyy"})
...
def sample_logtail_config(client, project, logstore) : 
    logtail_config = CommonRegLogConfigDetail("xxx_1-logtail", logstore, "http://cn-hangzhou-devcommon-intranet.sls.aliyuncs.com", "/apsara/xxx", "*.LOG", 
            "%Y-%m-%d %H:%M:%S", "xxx.*", "(.*)(.*)" , ["time", "value"])

    res = client.create_logtail_config(project, logtail_config)
    res.log_print()
    
    res = client.get_logtail_config(project, 'xxx_1-logtail')
    res.log_print()

    res = client.delete_logtail_config(project, "xxx_1-logtail")
    res.log_print()


def sample_apply_config(client, project):
    logtail_config = CommonRegLogConfigDetail("yyy_1-logtail", logstore, "http://cn-hangzhou-devcommon-intranet.sls.aliyuncs.com", "/apsara/xxx", "*.LOG", 
            "%Y-%m-%d %H:%M:%S", "xxx.*", "(.*)(.*)" , ["time", "value"])

    res = client.create_logtail_config(project, logtail_config)
    res = client.get_machine_group_applied_configs(project, "test-group")
    res.log_print()

    res = client.get_config_applied_machine_groups(project, "yyy_1-logtail")
    res.log_print()

    res = client.apply_config_to_machine_group(project, "yyy_1-logtail", "test-group")
    res.log_print()
    ...
    res = client.delete_logtail_config(project, "yyy_1-logtail")
    res.log_print()
